...
function goodVsEvil(good, evil){
    ...
    const allGood = good.split(' ')
          .map(element => Number(element))
          .reduce((allGood, races, index) => allGood + races * goodWorth[index], 0);
       
    const badWorth = [1,2, 2, 2, 3, 5, 10];
    const allBad = evil.split(' ')
          .map(element => Number(element))
          .reduce((allBad, races, index) => allBad + races * badWorth[index], 0);
  
    if (allGood === allBad) return "Battle Result: No victor on this battle field";
    else if (allGood > allBad) return "Battle Result: Good triumphs over Evil";
